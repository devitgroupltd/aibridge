...
async function waitForButton(page, topicSubstring, buttonText, { rounds = 8, intervalMs = 3000 } = {}) {
  const t0 = now();
  for (let i = 0; i < rounds; i++) {
    await openTopic(page, topicSubstring);
    await page.waitForTimeout(500);
    const btn = page.locator(".reply-markup-button", { hasText: buttonText }).last();
...
async function resolveAllPrompts(page, topicSubstring, { maxTaps = 5 } = {}) {
  const taps = [];
  for (let i = 0; i < maxTaps; i++) {
    const found = await waitForButton(page, topicSubstring, "✅ Allow");
    if (!found) break;
    await found.btn.click({ force: true });
    taps.push(found.elapsedMs);
    log(`tapped "✅ Allow" #${taps.length} for ${topicSubstring} after ${found.elapsedMs}ms wait`);
...
(async () => {
  ...
  if (slugs[2]) {
    ...
    if (permTaps.length === 0) log("FAIL: no permission card appeared for [RS3] within timeout");
  }
    ...
    JSON.stringify(
      {
        ...
        firstPermCardLatencyMs: permTaps[0] ?? null,
        allPermTapLatenciesMs: permTaps,
      },
      null,
      2,
    ),
