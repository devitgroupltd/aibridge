...
function fallbackDocument(root: string, filesChanged: number, untrackedFiles: string[]): DiffReviewResult {
  const raw = git(root, ["diff", "HEAD"]);
  const scrubbed = scrubSecrets(raw).text;
  return { kind: "document", diffText: scrubbed, filesChanged, untrackedFiles };
}
...
export function buildDiffReview(worktreeRoot: string, slug: string): DiffReviewResult {
  ...
  try {
    ...
    if (stashSha.length === 0) {
      return { kind: "empty", filesChanged: 0, untrackedFiles: status.untracked };
    }

    const filesChanged = git(root, ["diff", "--name-only", "HEAD", stashSha])
      .split("\n")
    ...
    try {
      ...
      return fallbackDocument(root, filesChanged, status.untracked);
    }
    ...
    return { kind: "link", url, filesChanged, untrackedFiles: status.untracked };
  } catch {
    try {
      return fallbackDocument(root, 0, []);
    } catch {
      return { kind: "empty", filesChanged: 0, untrackedFiles: [] };
    }
  }
}
