...
export const ABOUT_TOPICS: Record<string, { label: string; blurb: string; details: string }> = {
  ...
  bulk_rm: {
    ...
    blurb: "/remove can clear many dead rows at once instead of one at a time.",
    details: [
      "/remove --dead removes every session currently in the 'dead' state.",
      "/remove --prefix seowrite- removes every dead session whose slug starts with 'seowrite-'.",
      "/remove --all removes every session regardless of state (including live ones) - this is destructive, so it asks for a Yes/No confirm tap before doing anything, unlike the other two.",
      "",
  ...
  tuning: {
    ...
    details: [
      ...
      "",
      "/auto permission|answer [<slug>] [on|off] - let the Bridge resolve escalations for you instead",
      "of sending a card. Unlike /mode above, this works at the Bridge relay layer, so it applies",
      "however that session's own permission mode is set:",
      "  permission - auto-allows every permission prompt, including git commit/push, PR merge and",
      "    npm publish. The deny list (force-push, secret reads, rm -rf /) still hard-blocks - that is",
      "    the one thing it cannot bypass. Turning it on also clears any card already waiting.",
      "  answer - auto-picks the option Claude itself marked \"(Recommended)\". You still get the real",
      "    buttons when there's no clear recommendation, or when another option offers to",
      "    investigate/verify/hold off first - those stay yours to choose.",
      "Bare (no on|off) reports the current setting. Both reset to off whenever the Bridge restarts.",
      "  /auto permission|answer --all [on|off] - the same for every live session at once, behind the",
      "    same Yes/No confirm card /kill --all uses. Bare --all lists both settings per session.",
      "  /default permission|answer [on|off] - the same two toggles as a *new-session* default, so",
      "    every session created afterwards starts with them already on. This one does survive a",
      "    Bridge restart, and it doesn't touch sessions that are already running.",
      "",
