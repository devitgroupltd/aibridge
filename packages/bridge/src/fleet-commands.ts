...
export function renderHelp(): string {
  return [
    ...
    "  /restart - restart the Bridge daemon; confirm-gated (Yes/Cancel card) whenever any session",
    "    is live, since every one of them dies with the process - restarts immediately only if the",
    "    fleet is already all-dead",
    ...
    "    aibridge's own) restart the Bridge to pick up the fix (§5.9); restarts immediately unless",
    "    some *other* session is also live, in which case it's confirm-gated same as /restart - the",
    "    session that just merged is expected to die and cold-resume either way, so it doesn't count",
    ...
    "    same confirm-gate as /merge on its self-repo restart",
...
export function botCommandList(): { command: string; description: string }[] {
  return [
    ...
    { command: "restart", description: "Restart the Bridge daemon - confirm-gated whenever any session is live" },
    { command: "os", description: "Shut down or reboot the HOST MACHINE (not just the Bridge): /os shutdown|reboot|cancel - confirm-gated" },
    { command: "merge", description: "Merge a session's branch and run tests: /merge <slug> (restarts if it's aibridge's own repo, confirm-gated if another session is live)" },
